...
DevelopVersionString=`grep -E 'version.develop.*=' DesignPattern_Extension.podspec`
DevelopVersionNumber=`tr -cd 0-9 <<<"$DevelopVersionString"`
NewDevelopVersionNumber=$DevelopVersionNumber

TestVersionString=`grep -E 'version.test.*=' DesignPattern_Extension.podspec`
TestVersionNumber=`tr -cd 0-9 <<<"$TestVersionString"`
NewTestVersionNumber=$TestVersionNumber

GrayVersionString=`grep -E 'version.gray.*=' DesignPattern_Extension.podspec`
...
DevelopVersionLineNumber=`grep -nE 'version.develop.*=' DesignPattern_Extension.podspec | cut -d : -f1`
sed -i "" "${DevelopVersionLineNumber}s/${DevelopVersionNumber}/${NewDevelopVersionNumber}/g" DesignPattern_Extension.podspec
...
TestVersionLineNumber=`grep -nE 'version.test.*=' DesignPattern_Extension.podspec | cut -d : -f1`
sed -i "" "${TestVersionLineNumber}s/${TestVersionNumber}/${NewTestVersionNumber}/g" DesignPattern_Extension.podspec
echo "current test version is ${TestVersionNumber}, new version is ${NewTestVersionNumber}, line number is ${TestVersionLineNumber}"

GrayVersionLineNumber=`grep -nE 'version.gray.*=' DesignPattern_Extension.podspec | cut -d : -f1`
sed -i "" "${GrayVersionLineNumber}s/${GrayVersionNumber}/${NewGrayVersionNumber}/g" DesignPattern_Extension.podspec
echo "current gray version is ${GrayVersionNumber}, new version is ${NewGrayVersionNumber}, line number is ${GrayVersionLineNumber}"

VersionString=`grep -E 's.version.*=' DesignPattern_Extension.podspec`
...
VersionLineNumber=`grep -nE 's.version.*=' DesignPattern_Extension.podspec | cut -d : -f1`
sed -i "" "${VersionLineNumber}s/${VersionNumber}/${NewVersionNumber}/g" DesignPattern_Extension.podspec
...
pod repo push ${Repo} DesignPattern_Extension.podspec --verbose --allow-warnings --use-libraries --use-modular-headers
